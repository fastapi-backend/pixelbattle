...
async def get_token(request: OAuth2PasswordRequestForm = Depends()):
    user = await interface.get_model_filter(model=User,filter=request.username)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Invalid credentials')
    hashpasswdfromdb = await interface.get_model_filter_column(model=User,filter=request.username,column="hashed_password")
...
async def battle(current_user: schemas.UserBase = Depends(get_current_user)): 
    if current_user:
        return await interface.get_model(model=Battle)
    else:
        ...
        while True:
            ...
            if manager.broadcast:
                ...
                if pixel and color is not None:
                    battle = await interface.get_model_filter(model=Battle,filter=pixel)
                    if battle:
                        await interface.update_model(model=Battle,_id=pixel,data={"pixel":pixel,"color":color})
                        await manager.broadcast(data)
                    else:
                        await interface.set_battle_pixel_color(pixel=pixel,color=color)
                        await manager.broadcast(data)
    except Exception:
        await manager.send_personal_message("disconnect",websocket)
